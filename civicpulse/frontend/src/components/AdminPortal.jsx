...
export default function AdminPortal({ incidents, onStatusChange }) {
  ...
  const WARDS = [
    ...
    { id: 'Ward 12', label: 'Ward 12 - North Industrial' },
    { id: 'Ward 08', label: 'Ward 08 - South Suburbs' },
    { id: 'Ward 04', label: 'Ward 04 - East Tech Corridor' }
  ];
  ...
  const handleDispatchCrew = async (incident) => {
    const targetId = incident.incident_id || incident.id;
  ...
  return (
    <div className="space-y-6">
      {/* 1. Header */}
      <div className="bg-white p-6 sm:p-8 rounded-[32px] border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <select
            ...
            className="w-full md:w-64 bg-[#F4F8F6] border border-slate-200 text-xs font-bold text-slate-800 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-[#0B4D3C]"
          >
            {WARDS.map(w => (
      ...
      <div className="bg-white p-5 rounded-[28px] border border-slate-100 shadow-sm space-y-3">
        <div className="flex justify-between items-center text-xs">
          <span className="font-extrabold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
            <MapPin className="w-4 h-4 text-[#F97316]" /> Live Ward Incident Heatmap ({filteredIncidents.length} pinned)
          </span>
          <span className="text-slate-400 text-[11px]">BigQuery GIS Spatial Geometry</span>
        </div>
                      ...
                      <>
                        {!isInProgress && (
                          <button
                            ...
                            className="bg-[#0B4D3C] hover:bg-[#047857] text-white font-bold px-3 py-1.5 rounded-xl transition flex items-center gap-1 shrink-0 disabled:opacity-60"
                          >
                        ...
                        <button
                          ...
                          className="bg-[#10B981] hover:bg-emerald-600 text-slate-950 font-extrabold px-3 py-1.5 rounded-xl transition flex items-center gap-1 shrink-0 shadow-sm"
                        >
